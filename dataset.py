...
class LoadData(Dataset):
    def __init__(self, data_path, divide_days, time_interval, history_length, train_mode):
        ...
        #                                                       norm_dim=0)
        self.park_data = LoadData.get_park_data(park_file=self.data_path)
...
if __name__ == '__main__':
    ...
    #     testx = np.vstack((testx, single_test_data['park_x'].unsqueeze(0).numpy()))
    #     testy = np.vstack((testy, single_test_data['park_y'].unsqueeze(0).numpy()))
    # print(testx.shape)
    # print(testy.shape)
